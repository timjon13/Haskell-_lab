...
power2 n k 
	...
	--assigment 4
--a)
	-- To see if the statments are true or not we have to check for some values which are not in the Range of the two functions such as if it bettween  1 to 100.Might also check for the value of 0 and something which is outside the value of the list such as checking for the value of -1000.For the value of -1000 should give that the value is not the same.
...
comparePower1 n k= power n k == power1 n k -- the funcation of the whole statement checks if 
...
--c)
testAllPowerFunctions::Bool-- testpowerfunctions takes an bool to check if the value of the two statements are the same 
testAllPowerFunctons = and[comparePower1 n k == comparePower2 n k -- The and function is the boolean fucntion that checks if the satement for function comparepower1 n k is the same for caomparePower2 n k 
